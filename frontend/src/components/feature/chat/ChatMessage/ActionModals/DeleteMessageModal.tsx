...
export const DeleteMessageModal = ({ onClose, message }: DeleteMessageModalProps) => {
    ...
    return (
        <>
            <AlertDialog.Title>
                {__('Delete')} {message.is_thread ? __('Thread') : __('Message')}
            </AlertDialog.Title>
            ...
            <Flex direction={'column'} gap='2'>
                ...
                {message.is_thread ? <Text size='2'>{__('This message is a thread, deleting it will delete all messages in the thread.')}</Text> :
                    <Text size='2'>{__('Are you sure you want to delete this message? It will be deleted for all users.')}</Text>}
            </Flex>
            ...
            <Flex gap="3" mt="4" justify="end">
                ...
                <AlertDialog.Action>
                    <Button variant="solid" color="red" onClick={onSubmit} disabled={deletingDoc}>
                        ...
                        {deletingDoc ? __("Deleting") : __("Delete")}
                    </Button>
                </AlertDialog.Action>
            </Flex>
        </>
